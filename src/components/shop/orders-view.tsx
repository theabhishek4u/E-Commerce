...
export function OrdersView() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="flex items-center gap-3 mb-6"><Button variant="ghost" size="icon" onClick={() => setCurrentView('home')}><ArrowLeft className="h-5 w-5" /></Button><h1 className="text-2xl font-bold font-heading">My Orders</h1></div>
      <Card className="mb-8 border-border/30 bg-white/80 backdrop-blur-sm">
        <CardContent className="p-6">
          <h2 className="font-semibold font-heading mb-3">Find your orders</h2>
          <form onSubmit={handleSearch} className="flex gap-3">
            ...
            <Button type="submit" className="bg-gradient-to-r from-blue-600 to-blue-700 text-white" disabled={loading}>{loading ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : <><Search className="h-4 w-4 mr-2" />Find</>}</Button>
          </form>
        </CardContent>
      </Card>
                ...
                <CardContent className="p-0">
                  <div className="bg-slate-50 p-4 flex flex-wrap items-center gap-3 justify-between">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div><p className="text-xs text-muted-foreground">Total</p><p className="font-bold text-sm text-blue-700">{formatINR(order.totalAmount)}</p></div>
                    </div>
                  ...
                  <div className="p-4 flex items-center gap-3">
                    ...
                    <Button variant="ghost" size="sm" className="text-blue-700" onClick={() => navigateToOrder(order.id, order.orderNumber)}><Eye className="h-4 w-4 mr-1" />View</Button>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
