  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-4 sm:py-6 pb-28 sm:pb-6">
      ...
      <nav className="flex items-center gap-1.5 text-xs sm:text-sm mb-5 sm:mb-7" aria-label="Breadcrumb">
        <button onClick={() => setCurrentView('home')} className="text-muted-foreground hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-colors font-medium">Home</button>
        <ChevronRight className="h-3 w-3 text-muted-foreground/40 dark:text-slate-600 shrink-0" />
        {category && (
          <>
            <button onClick={() => { useShopStore.getState().setSelectedCategory(category.slug); setCurrentView('home'); }} className="text-muted-foreground hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 transition-colors font-medium">{category.name}</button>
            <ChevronRight className="h-3 w-3 text-muted-foreground/40 dark:text-slate-600 shrink-0" />
          </>
        )}
        <span className="text-blue-600 dark:text-blue-400 font-semibold truncate max-w-[200px] sm:max-w-xs">{product.name}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10">
        ...
        <div className="space-y-3 sm:space-y-4">
          <motion.div
            className="aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 border border-slate-200/60 dark:border-slate-700/60 cursor-crosshair relative"
              ...
              {product.images.map((img, idx) => (
                <button key={idx} onClick={() => setSelectedImage(idx)}
                  className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden shrink-0 transition-all duration-300 ${
                    selectedImage === idx
                      ? 'ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-slate-900 scale-[1.05]'
        ...
        <div className="space-y-4 sm:space-y-5">
          ...
          <div>
            {product.brand && (
              <p className="text-xs sm:text-sm font-bold uppercase tracking-[0.15em] mb-1 sm:mb-1.5 bg-gradient-to-r from-blue-600 to-cyan-500 dark:from-blue-400 dark:to-cyan-400 bg-clip-text text-transparent">
                {product.brand}
              </p>
            )}
            {category && <Badge variant="secondary" className="mb-2 text-[10px] sm:text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-0">{category.name}</Badge>}
            <h1 className="text-2xl sm:text-3xl lg:text-[2rem] font-extrabold font-heading leading-tight tracking-tight dark:text-slate-100">{product.name}</h1>
          </div>
          ...
          {/* ── Price Card ── */}
          <div className="bg-gradient-to-br from-slate-50 via-white to-blue-50/40 dark:from-slate-800 dark:via-slate-900 dark:to-blue-950/40 rounded-2xl p-4 sm:p-5 space-y-2 sm:space-y-2.5 border border-blue-100/60 dark:border-slate-700/60">
            <div className="flex items-baseline gap-2 sm:gap-3 flex-wrap">
              <span className="text-3xl sm:text-4xl font-extrabold tracking-tight dark:text-slate-100">{formatINR(product.price)}</span>
              {product.originalPrice && product.originalPrice > product.price && (
                <>
                  <span className="text-base sm:text-lg text-muted-foreground dark:text-slate-500 line-through decoration-red-400/60">{formatINR(product.originalPrice)}</span>
            ...
            <p className="text-xs text-muted-foreground dark:text-slate-500">Inclusive of all taxes</p>
            <div className="pt-1">
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-medium">
                EMI from <span className="font-bold text-blue-600 dark:text-blue-400">{formatINR(emiAmount)}/month</span>
              </p>
            </div>
          </div>
          ...
          {product.stock > 0 && (
            <div className="hidden sm:flex items-center gap-4">
              ...
              <div className="flex items-center bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full overflow-hidden">
                <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full hover:bg-slate-200/60 dark:hover:bg-slate-700/60" onClick={() => setQuantity(Math.max(1, quantity - 1))} disabled={quantity <= 1}>
                  <Minus className="h-4 w-4" />
                </Button>
                <span className="w-10 text-center font-bold text-sm tabular-nums dark:text-slate-200">{quantity}</span>
                <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full hover:bg-slate-200/60 dark:hover:bg-slate-700/60" onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} disabled={quantity >= product.stock}>
          ...
          <div className="hidden sm:flex gap-3">
            ...
            <motion.div whileTap={{ scale: 0.85 }} animate={wishlistAnimating ? { scale: [1, 1.3, 1] } : { scale: 1 }} transition={{ duration: 0.3 }}>
              <Button size="lg" variant="outline" className="h-13 w-13 shrink-0 border-slate-200 dark:border-slate-700 hover:border-red-200 dark:hover:border-red-800 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors" onClick={handleToggleWishlist}>
                <Heart className={`h-5 w-5 transition-all ${inWishlist ? 'fill-red-500 text-red-500' : 'text-slate-400'}`} />
              </Button>
            </motion.div>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button size="lg" variant="outline" className="h-13 w-13 shrink-0 border-slate-200 dark:border-slate-700 hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:border-blue-200 dark:hover:border-blue-700 transition-colors" onClick={() => toast.success('Link copied!')}>
          ...
          <Separator className="bg-slate-100 dark:bg-slate-700/40" />

          {/* ── Trust Badges ── */}
          <div className="space-y-2.5 sm:space-y-3">
            {[
              { icon: Truck, t: 'Free Delivery', d: ' on orders above ₹499', bg: 'bg-blue-50 dark:bg-blue-950/30', iconColor: 'text-blue-600 dark:text-blue-400', border: 'border-blue-100 dark:border-blue-900/30' },
              { icon: Shield, t: 'Secure Payment', d: ' · UPI, Cards, COD', bg: 'bg-emerald-50 dark:bg-emerald-950/30', iconColor: 'text-emerald-600 dark:text-emerald-400', border: 'border-emerald-100 dark:border-emerald-900/30' },
              { icon: RotateCcw, t: '7-Day Returns', d: ' · Easy returns', bg: 'bg-amber-50 dark:bg-amber-950/30', iconColor: 'text-amber-600 dark:text-amber-400', border: 'border-amber-100 dark:border-amber-900/30' },
            ].map((item, i) => (
              <div key={i} className={`flex items-center gap-3 text-xs sm:text-sm p-2.5 sm:p-3 rounded-xl ${item.bg} border ${item.border}`}>
                ...
                <div>
                  <span className="font-semibold dark:text-slate-200">{item.t}</span>
                  <span className="text-muted-foreground dark:text-slate-400">{item.d}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* ── Description ── */}
          <div className="bg-slate-50/60 dark:bg-slate-800/60 rounded-2xl p-4 sm:p-5 border border-slate-100 dark:border-slate-700/60">
            <h3 className="font-bold text-base sm:text-lg font-heading mb-2 sm:mb-3 flex items-center gap-2 dark:text-slate-200">
              <Award className="h-5 w-5 text-blue-600 dark:text-blue-400" /> Description
            </h3>
            <p className="text-muted-foreground dark:text-slate-400 text-sm leading-relaxed whitespace-pre-line">{product.description}</p>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <h3 className="font-bold text-base sm:text-lg font-heading flex items-center gap-2 dark:text-slate-200">
                <MessageSquare className="h-5 w-5 text-blue-600 dark:text-blue-400" /> Reviews ({reviews.length})
              </h3>
            </div>
            {reviews.length === 0 ? (
              <div className="text-center py-8 bg-slate-50/60 dark:bg-slate-800/60 rounded-2xl border border-slate-100 dark:border-slate-700/60">
                <MessageSquare className="h-10 w-10 mx-auto text-muted-foreground/20 mb-2" />
                <p className="text-sm text-muted-foreground dark:text-slate-400">No reviews yet. Be the first to review!</p>
              </div>
            ) : (
              <div className="space-y-3 sm:space-y-4 max-h-80 sm:max-h-96 overflow-y-auto pr-1">
                {reviews.map((review) => (
                  <div key={review.id} className="p-3.5 sm:p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700/60 transition-colors">
                    <div className="flex items-center justify-between mb-2.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-xs sm:text-sm font-semibold dark:text-slate-200">{review.userName}</p>
                          {review.verified && (
                            <Badge className="bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 text-[10px] px-1.5 py-0 h-4 mt-0.5">
                      ...
                      <div className="flex items-center gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={`h-3.5 w-3.5 ${i < review.rating ? 'fill-amber-400 text-amber-400' : 'text-gray-200 fill-gray-200 dark:text-slate-600 dark:fill-slate-600'}`} />
                        ))}
                      </div>
                    </div>
                    {review.title && <p className="font-semibold text-xs sm:text-sm mb-1 dark:text-slate-200">{review.title}</p>}
                    <p className="text-xs sm:text-sm text-muted-foreground dark:text-slate-400 leading-relaxed">{review.comment}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {relatedProducts.length > 0 && (
        <section className="mt-10 sm:mt-14">
          <div className="flex items-center gap-3 mb-5 sm:mb-7">
            ...
            <h2 className="text-lg sm:text-xl font-bold font-heading flex items-center gap-2 dark:text-slate-200">
